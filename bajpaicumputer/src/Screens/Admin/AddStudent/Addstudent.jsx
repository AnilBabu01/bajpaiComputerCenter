...
import Updatebranch from "./Updatestudent";
...
export default function Addstudent({ setshowadmin }) {
  ...
  const getgame = () => {
    try {
      setshowloader(true);
      serverInstance("student", "get").then((res) => {
        if (res?.status) {
  ...
  const filter = () => {
    try {
      setshowloader(true);
      serverInstance("/searchstudent", "post", {
        date: date,
        rollno: rollno,
        branch: branch,
      }).then((res) => {
